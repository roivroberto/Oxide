...
fn should_terminate_containment(state: &ActorState) -> bool {
...
mod tests {
    ...
    #[test]
    fn reaped_root_with_pending_streams_requires_containment_termination() {
        let mut state = ActorState::new(WorkerSessionId(1), &config()).unwrap();
        state.child_status = Some(Some(0));
        state.stdout_done = false;
        state.stderr = None;

        assert!(should_terminate_containment(&state));
    }

    #[test]
    fn actor_receiver_disconnects_before_saturated_producers_are_joined() {
        let (actor, messages) = mpsc::sync_channel(1);
        actor.send(ActorMessage::Close).unwrap();
        ...
        attempt_rx
            ...
            .expect("producer reached saturated terminal send");
        thread::yield_now();
        assert!(!blocked_producer.is_finished());
        let idle_writer = thread::spawn(|| {});
        let idle_stderr = thread::spawn(|| {});
        let (writer_tx, _writer_rx) = mpsc::sync_channel::<WriteRequest>(1);
        let (done_tx, done_rx) = mpsc::sync_channel(1);

        thread::spawn(move || {
            join_actor_producers(
                messages,
                writer_tx,
                idle_writer,
                blocked_producer,
        ...
        done_rx
            .recv_timeout(Duration::from_secs(1))
            .expect("producer joins complete after receiver disconnect");
    }
